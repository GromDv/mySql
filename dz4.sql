...
SELECT SUM(cnt) as 'Всего лайков до 12 лет'
FROM (
	SELECT 
	#	CONCAT(u.firstname, ' ', u.lastname, ";  ",p.birthday) AS 'владелец медиа',	
	#	datediff(curdate(), p.birthday)/365 < 12 as 'age',
		COUNT(l.id) as cnt
	FROM media m
...
SELECT IF (	(	SELECT 
				...
				JOIN profiles p ON u.id = p.user_id
				WHERE p.gender = 'm'
			) > 0, 'Женщины', 'Мужчины') as 'Кто больше?';
...
SELECT 
	@f_likes as 'Женщины',
    @m_likes as 'Мужчины',
	IF (@f_likes - @m_likes > 0, 'Женщины', 'Мужчины') as 'Кто больше?';
...
-- 4. Пусть задан некоторый пользователь ('1'). Из всех друзей этого пользователя найдите человека,
...
WHERE m.to_user_id = '1' AND m.from_user_id IN (
	SELECT IF ( initiator_user_id = '1', target_user_id, initiator_user_id)
...
SELECT 
	IF ( f.initiator_user_id = '1', f.target_user_id, f.initiator_user_id) as friend_id,
    IF ( f.initiator_user_id = '1', CONCAT(u2.firstname, ' ', u2.lastname), CONCAT(u.firstname, ' ', u.lastname)) as 'friend name'
FROM friend_requests f
JOIN users u ON u.id = f.initiator_user_id
JOIN users u2 ON u2.id = f.target_user_id
WHERE `status` = 'approved' AND (f.initiator_user_id = '1' OR f.target_user_id = '1')
;

-- Лидеры по отправлению сообщений пользователю "1"
SELECT COUNT(m.id) as amount,
	m.from_user_id,
