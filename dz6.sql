...
-- переместить любого (одного) пользователя из таблицы users в таблицу users_old. (использование транзакции
...
DELIMITER //
CREATE PROCEDURE sp_user_move(u_id INT, OUT tr_res varchar(100))
BEGIN
	DECLARE `_rollback` BIT DEFAULT b'0';
	DECLARE code varchar(100);
	DECLARE error_string varchar(100); 
	DECLARE if_user_exists INT;
	...
	START TRANSACTION;
		SET if_user_exists = (SELECT COUNT(id) FROM users WHERE id = u_id);
		IF if_user_exists = 0 THEN
			SET `_rollback` = b'1';
			SET code = '1';
			SET error_string = 'Нет такого пользователя!';
		END IF;
	...
	IF `_rollback` THEN
		SET tr_res = CONCAT('Код ошибки: ', code, ' Сообщение: ', error_string);
		ROLLBACK;
	ELSE
		SET tr_res = 'Успешно!';
		COMMIT;
...
# Проверяем работу на пользователе 4
CALL sp_user_move(4,@tr_res);
SELECT @tr_res;
...
-- до 18:00 функция должна возвращать фразу "Добрый день", с 18:00 до 00:00 — "Добрый вечер", с 00:00
--  до 6:00 — "Доброй ночи".
DROP FUNCTION IF EXISTS my_hello;
DELIMITER //
CREATE FUNCTION my_hello(hadd INT)
RETURNS VARCHAR(20) NO SQL
#RETURNS BIGINT NO SQL
BEGIN
	DECLARE cur_time BIGINT;
	DECLARE hello_text VARCHAR(20);

	SET cur_time = TIME_TO_SEC(DATE_ADD(CURRENT_TIME(),INTERVAL hadd HOUR));

	IF cur_time < 21600 THEN
		SET hello_text = 'Доброй ночи';
	ELSEIF cur_time < 43200 THEN
		SET hello_text = 'Доброе утро';
	ELSEIF cur_time < 64800 THEN
...
# Для проверки можно ввести в виде параметра сдвиг (+-в часах) относительно текущего времени.
SELECT my_hello(2);
...
# триггер для users
DROP TRIGGER if exists my_log_users;
DELIMITER $$
$$
CREATE TRIGGER my_log_users
AFTER INSERT
ON users FOR EACH ROW
BEGIN 
	INSERT INTO logs (c_table, rec_id_pk) VALUES ('users', NEW.id);
END$$
DELIMITER ;

# триггер для communities
DROP TRIGGER if exists my_log_communities;
DELIMITER $$
$$
CREATE TRIGGER my_log_communities
AFTER INSERT
ON communities FOR EACH ROW
BEGIN 
	INSERT INTO logs (c_table, rec_id_pk) VALUES ('communities', NEW.id);
END$$
DELIMITER ;

# триггер для messages
DROP TRIGGER if exists my_log_messages;
DELIMITER $$
$$
CREATE TRIGGER my_log_messages
AFTER INSERT
ON messages FOR EACH ROW
BEGIN 
	INSERT INTO logs (c_table, rec_id_pk) VALUES ('messages', NEW.id);
END$$
DELIMITER ;
