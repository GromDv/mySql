...
CREATE OR REPLACE VIEW v_users_before_20yo AS
SELECT 
	id,
...
WHERE datediff(curdate(), p.birthday)/365 < 20;

SELECT * 
FROM v_users_before_20yo;

-- Найдите кол-во, отправленных сообщений каждым пользователем и выведите 
-- ранжированный список пользователь, указав указать имя и фамилию пользователя, 
...
(SELECT 
	CONCAT(u.firstname, ' ', u.lastname) AS 'user',
	COUNT(m.id) OVER(PARTITION BY m.from_user_id) AS 'num_mess'
FROM messages m 
JOIN users u ON u.id = m.from_user_id)
SELECT
	`user`,
	`num_mess`,
	DENSE_RANK() OVER(ORDER BY `num_mess` DESC) as 'rank'
FROM users_mess;
...
-- а) временная таблица
DROP TABLE IF EXISTS tbl_timedif;
CREATE TEMPORARY TABLE tbl_timedif
SELECT
	id,
	created_at,
	LAG(created_at, 1, 0) OVER(ORDER BY created_at) as prev,
	LEAD(created_at, 1, 0) OVER(ORDER BY created_at) as nxt
FROM messages m
;
SELECT
	id,
	created_at,
	TIMEDIFF(created_at, prev) as 'от предыдущего',
	TIMEDIFF(created_at, nxt) as 'до следующего'
FROM tbl_timedif; 
